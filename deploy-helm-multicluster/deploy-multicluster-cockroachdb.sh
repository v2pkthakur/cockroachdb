...
export cluster3=azure-cluster-08 

export cluster1_context=aws-0
export cluster2_context=aws-1
export cluster3_context=azure-08
...
# Deploy on Cluster1 
oc --context ${cluster1_context} new-project multicluster-cockroachdb
...
export uid=$(oc --context ${cluster1_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.uid-range}'|sed 's/\/.*//')
export guid=$(oc --context ${cluster1_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.supplemental-groups}'|sed 's/\/.*//')
export cluster=${cluster1}
envsubst < /tmp/values.templ.yaml > /tmp/cluster1-values.yaml
helm --kube-context ${cluster1_context} upgrade multicluster-cockroachdb .  -i --create-namespace -n multicluster-cockroachdb -f /tmp/cluster1-values.yaml



# Deploy on Cluster2 
oc --context ${cluster2_context} new-project multicluster-cockroachdb
...
export uid=$(oc --context ${cluster2_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.uid-range}'|sed 's/\/.*//')
export guid=$(oc --context ${cluster2_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.supplemental-groups}'|sed 's/\/.*//')
export cluster=${cluster2}
envsubst < /tmp/values.templ.yaml > /tmp/cluster2-values.yaml
helm --kube-context ${cluster1_context} upgrade cockroachdb .  -i --create-namespace -n multicluster-cockroachdb -f /tmp/cluster2-values.yaml


# Deploy on Cluster3
oc --context ${cluster3_context} new-project multicluster-cockroachdb
...
export uid=$(oc --context ${cluster3_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.uid-range}'|sed 's/\/.*//')
export guid=$(oc --context ${cluster3_context} get project multicluster-cockroachdb -o jsonpath='{.metadata.annotations.openshift\.io/sa\.scc\.supplemental-groups}'|sed 's/\/.*//')
export cluster=${cluster3}
envsubst < /tmp/values.templ.yaml > /tmp/cluster3-values.yaml
helm --kube-context ${cluster3_context} upgrade cockroachdb .  -i --create-namespace -n multicluster-cockroachdb -f /tmp/cluster3-values.yaml
